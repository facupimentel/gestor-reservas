    ...
    <div className="min-h-screen bg-paper font-body">
      <header className="border-b border-ink/10">
        <div className="max-w-3xl mx-auto px-6 py-6 flex items-center justify-between">
          <div>
            <p className="text-xs tracking-[0.2em] uppercase text-brick-rose font-medium">Reservá tu turno</p>
            <h1 className="font-display text-2xl font-semibold text-ink-dark mt-1">
              {config?.businessName || 'Mi Negocio'}
            </h1>
          </div>
          <Link
            to="/admin/login"
            className="text-xs text-brick-rose hover:text-ink/70 transition-colors font-body"
          >
            Panel admin
          </Link>
        </div>
      ...
      <main className="max-w-3xl mx-auto px-6 py-10">
        ...
        {step === 1 && (
          <section>
            <h2 className="font-display text-xl text-ink-dark mb-1">Elegí un servicio</h2>
            <p className="text-ink-dark text-sm mb-6">Seleccioná lo que querés reservar.</p>
              ...
              {services.map((s) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="font-display text-lg text-ink group-hover:text-brick transition-colors">
                      {s.name}
                    </h3>
                  ...
                  <p className="font-mono text-brick mt-3">{formatMoney(s.price)}</p>
                </button>
              ))}
              {services.length === 0 && (
        ...
        {step === 2 && selectedService && (
          <section>
            ...
            <button
              ...
              onClick={() => setStep(3)}
              className="mt-8 bg-brick text-paper-light px-5 py-2.5 rounded-md font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:bg-brick-dark transition-colors"
            >
              Continuar
            </button>
          </section>
            ...
            <form onSubmit={handleConfirm} className="space-y-4 max-w-md">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-ink/20 rounded-md px-3 py-2 bg-paper-light focus:outline-none focus:ring-2 focus:ring-brick/40"
                />
              </div>
              <div>
              ...
              <button
                disabled={submitting}
                className="bg-brick text-paper-light px-5 py-2.5 rounded-md font-medium disabled:opacity-50 hover:bg-brick-dark transition-colors"
              >
        ...
        {step === 4 && confirmed && (
          <section className="flex flex-col items-center py-6">
            <div className="ticket-stub bg-ink text-paper-light rounded-lg w-full max-w-sm overflow-hidden">
              <div className="p-6">
                <p className="text-xs uppercase tracking-[0.2em] text-brass font-mono">Turno confirmado</p>
                <h2 className="font-display text-2xl mt-2">{confirmed.serviceName}</h2>
                <div className="mt-4 space-y-1 font-mono text-sm text-paper-light/80">
                  <p>
                    {WEEKDAY_LABELS[new Date(confirmed.start).getDay()]} {confirmed.date}
                  </p>
                  <p>{confirmed.startTime} – {confirmed.endTime} hs</p>
                  <p>{formatMoney(confirmed.price)}</p>
                </div>
              </div>
              <div className="border-t border-dashed border-paper-light/30 px-6 py-3 flex items-center justify-between text-xs font-mono text-paper-light/50">
                <span>Reserva #{confirmed._id.slice(-6).toUpperCase()}</span>
                <span>{confirmed.clientName}</span>
              </div>
            </div>
            <p className="text-ink/50 text-sm mt-6 text-center max-w-sm">
